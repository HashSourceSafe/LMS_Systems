...
function CreateTH() {
    try {
        var m_th = document.createElement('th');
        return m_th;
    }
    catch (err) {
        alert("CreateTH()- Error In JScript");
    }
}

function AddHeading(p_row_ref, p_value, p_class, p_size,p_colspan) {
    try {
        ...
        //p_col_ref = CreateColumn();
        p_col_ref = CreateTH(); //New Add for Header

        p_col_ref.setAttribute('colspan', p_colspan);
        p_col_ref.setAttribute('class', p_class);
        p_col_ref.setAttribute('size', p_size);
        p_col_ref.style.border = "1px solid #18A2D5";
...
function SetGridRowBackColorWithText(p_grid_name, p_text, p_text_ctrl_name,p_ctrl_name, p_color_code) {
    try {
        m_grid_ctrl_ref = window.document.getElementById(p_grid_name);
        if (m_grid_ctrl_ref == null) {
            alert("Grid Error: " + p_grid_name);
        ...
        for (iRow = 0; iRow < m_grid_ctrl_ref.rows.length; iRow++) {
            ...
            m_text_ctrl_name = p_grid_name + "_ctrl_" + p_text_ctrl_name + "_" + iRow;
            m_text_ctrl_name = m_text_ctrl_name.toUpperCase();
            m_text_ctrl_ref = window.document.getElementById(m_text_ctrl_name);


            if (m_ctrl_ref != null) {
                if (m_text_ctrl_ref.value == p_text) {
                    m_ctrl_ref.style.backgroundColor = p_color_code;
                    m_ctrl_ref.parentNode.style.backgroundColor = p_color_code;

                    m_ctrl_ref.style.color = "white";

                    //m_ctrl_ref.style.textDecoration = "underline";
                    m_ctrl_ref.style.fontSize = "14px";
                    //m_ctrl_ref.style.fontWeight = "bold";
                }
            }
        ...
        alert("SetGridRowBackColorWithText()- Error In JScript");
    }
}
