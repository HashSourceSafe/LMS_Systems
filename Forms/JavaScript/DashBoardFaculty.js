...
function GetFacultyStatData() {
    ...
    try {
        ...
        ShowWaitMsgWnd("Please Wait - Loading Faculty Attd. Chart");
        WebServiceMasters.GetDataFromMasterDb("NA", m_field, m_sp_param, "Proc_Disp_Weekly_Faculty_Average_Attd", 0,
                                   OnComplete_GetFacultyStatData,
                                   OnError_GetFacultyStatData,
                                   OnTimeOut_GetFacultyStatData);
    }
    catch (err) {
        alert("GetFacultyStatData() - JScript Error");
    }
}
function OnComplete_GetFacultyStatData(arg) {
    DestroyWaitMsgWnd();
    if (arg.m_Count == -1) {
        alert('Error in WebMethod - GetFacultyStatData');
    }
    else {
        m_arr = arg.m_List;
        CreateFacultyAttdDetails();

        //GetDeptWiseBarChartData(0);

    }
}
function OnError_GetFacultyStatData(arg) {
    alert("error has occured: " + arg._message);
}
function OnTimeOut_GetFacultyStatData(arg) {
    alert("Time Out");
}
//----------------------------------

function CreateFacultyAttdDetails() {
    try {
        ...
        for (iCtr = 0; iCtr <= m_arr.length - 1; iCtr++) {
            ...
            m_total_class = m_arr[iCtr][5];
            m_curr_present = m_arr[iCtr][7];
            m_curr_absent = m_arr[iCtr][8];
            m_curr_date = m_arr[iCtr][9];
            ...
            if (parseFloat(m_total_class) > 0) {
                m_value = "<span class=''>Given</span><br>" + m_curr_present + "%";
            }
            else {
                m_value = "<span class=''>Given</span><br>NA";
            }
            ...
            if (parseFloat(m_total_class) > 0) {
                m_value = "<span class=''>Pending</span><br>" + m_curr_absent + "%";
            }
            else {
                m_value = "<span class=''>Pending</span><br>NA";
            }
        ...
        //alert("CreateFacultyAttdDetails() - JScript Error");
    }
}
...
function ShowDetail(p_date) {
    ...
    cntxt_college_id = GetValueByCtrlName("cntxt_college_id");
    m_url = "frmDashBoardFacultyDet.aspx?date=" + p_date;
...
function LoadUrl(p_college) {
    ...
    m_url = "frmDashBoardFaculty.aspx?college_id=" + p_college + "&is_call_from_mis=" + is_call_from_mis;
    window.open(m_url, "_parent", "location=0,menubar=0,resizable=1,status=0,titlebar=0,menubar=0,top=0,left=0");
}

function SwitchWnd() {
    is_call_from_mis = GetValueByCtrlName("is_call_from_mis");
    m_url = "frmDashBoard.aspx?college_id=0" + "&is_call_from_mis=" + is_call_from_mis;
    window.open(m_url, "_parent", "location=0,menubar=0,resizable=1,status=0,titlebar=0,menubar=0,top=0,left=0");
...
function GetDeptWiseBarChartData(p_day) {
    ...
    try {
        ...
        SetInnerHtml("m_bar_chart_caption", "Faculty Department Wise Bar Chart - " + dtp_curr_date);

        var m_sp_param = new Array();
        m_sp_param.push("@p_disp_type,INT," + "5");
        m_sp_param.push("@P_COLLEGE_ID,INT," + cntxt_college_id);
        ...
        var m_field = new Array();
        m_field.push("fac_dept");
        m_field.push("present%");
        m_field.push("absent%");
        m_field.push("attd_enter%");

        ShowWaitMsgWnd("Please Wait- Loading Course-Stream Chart");
        WebServiceMasters.GetDataFromMasterDb("NA", m_field, m_sp_param, "Proc_Get_Faculty_Attd_Percentage_New", 0,
...
function OnComplete_GetDeptWiseBarChartData(arg) {
    DestroyWaitMsgWnd();
    if (arg.m_Count == -1) {
        alert('Error in WebMethod - GetDeptWiseBarChartData');
    }
    else {
...
function drawBarChart(p_arr) {
    ...
    m_arr2.push("Department");
    m_arr2.push("Entry Pending(%)");
    //m_arr2.push("Absent(%)");
    m_arr2.push("Student Present(%)");
    m_arr1.push(m_arr2);

    for (iCtr = 0; iCtr <= p_arr.length - 1; iCtr++) {
        var m_arr2 = new Array();

        m_attd_enter = p_arr[iCtr][3];
        m_present = p_arr[iCtr][1];
        m_absent = p_arr[iCtr][2];
