...
var g_TemplateArrMergeHeading = new Array();
...
function DynamicHtmlTemplate_CreateColumn(p_obj_arr,p_obj_count, p_KeyName) {
    try {
        ...
        m_table = CreateTable(g_grid_name, "table table-bordered table-vertical-middle nomargin table-striped tableFixHead"); //table_view_dynamic tableFixHead

        m_table_body = CreateBody();
        m_table_heading = CreateTheard(); //New Add for Header
        m_table_heading.setAttribute('id', p_KeyName + '_heading');


        var m_param = new Array();
        var m_param_arr = new Array();

        //-----Creating Merge Heading 

        if (g_TemplateArrMergeHeading[g_curr_index] != null) {
            if (g_TemplateArrMergeHeading[g_curr_index] != "NA") {
                m_row = CreateRow();
                m_row.setAttribute('id', p_KeyName + '_heading__merge_row');

                for (nRow = 0; nRow < g_TemplateArrMergeHeading[g_curr_index].length; nRow++) {
                    //alert(g_TemplateArrMergeHeading[g_curr_index - 1][nRow]);

                    m_TempStr = g_TemplateArrMergeHeading[g_curr_index][nRow];
                    m_param_arr = m_TempStr.split(",");

                    if (m_param_arr[1] == "L") {
                        //m_css = "text_box_heading_left";
                        m_css = "text-left bg-primary text-white no-border line-hight24";
                    }
                    else if (m_param_arr[1] == "R") {
                        //m_css = "text_box_heading_right";
                        m_css = "text-right  bg-primary text-white no-border line-hight24";
                    }
                    else if (m_param_arr[1] == "C") {
                        //m_css = "text_box_heading_center";
                        m_css = "text-center bg-primary text-white no-border line-hight24";
                    }
                    else {
                        //m_css = "text_box_heading_left";
                        m_css = "text-left bg-primary text-white no-border line-hight24";
                    }
                    AddHeading(m_row, m_param_arr[0], m_css, m_param_arr[2], m_param_arr[3]);
                }
                AddRow(m_table_heading, m_row); //New Add for Header
                AddRow(m_table, m_table_heading); //New Add for Header
            }
        ...
        //-----Creating Column Heading 1
        m_row = CreateRow();
        m_row.setAttribute('id', p_KeyName+'_heading_row');
        ...
        for (nRow = 0; nRow < g_TemplateArrGetDataBaseField[g_curr_index].length; nRow++) {
            ...
            if (m_param_arr[2] != "hidden_textbox") {
                ...
                AddHeading(m_row, m_param_arr[1], m_css, m_param_arr[4],1);
            }
        ...
        for (nRow = 0; nRow < g_TemplateArrGetDataBaseField[g_curr_index].length; nRow++) {
            ...
            if (m_param_arr[3] == "L") {
                if (m_param_arr[2] == "enable_textbox" || m_param_arr[2] == "enable_textbox_multiline") {
                    ...
                    m_css = "text_box text-left pencil";
                }
                else {
                    //m_css = "text_box";
                    m_css = "text_box text-left";
                }
            }
            else if (m_param_arr[3] == "R") {
                if (m_param_arr[2] == "enable_textbox" || m_param_arr[2] == "enable_textbox_multiline") {
                    //m_css = "text_box_number pencil";
                    m_css = "text_box text-right pencil";
                }
                else {
                    //m_css = "text_box_number";
                    m_css = "text_box text-right";
                }
            }
            else if (m_param_arr[3] == "C") {
                if (m_param_arr[2] == "enable_textbox" || m_param_arr[2] == "enable_textbox_multiline") {
                    //m_css = "text_box_center pencil";
                    m_css = "text_box text-center pencil";
                }
                else {
                    //m_css = "text_box_center";
                    m_css = "text_box text-center";
                }
            }
            else {
                if (m_param_arr[2] == "enable_textbox" || m_param_arr[2] == "enable_textbox_multiline") {
        ...
        document.getElementById(g_TemplateArrRootContainer[g_curr_index]).scrollTo(0, -2000);
...
function DynamicHtmlTemplate_MergeHeading(p_Val) {
    try {
        g_TemplateArrMergeHeading.push(p_Val);
    }
    catch (err) {
        alert("DynamicHtmlTemplate_MergeHeading() - Error in JScript");
    }
}