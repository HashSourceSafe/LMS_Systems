...
function ConfirmMsg(p_var) {
    var m_val = window.confirm(p_var);
    return m_val;
}
...
//Added By Sujit
function ConvertdateYyyyMmDd(p_val) {
    var m_val = p_val;
...
function ShowMsgWnd(pMesg) {
    try {
        ...
        m_but.setAttribute('class', "close1");
...
function GetDateDiff(p_from_date,p_to_date) {
    try {
        var m_Date1 = new Date(GetYear(p_from_date), GetMonth(p_from_date)-1, GetDay(p_from_date));
        var m_Date2 = new Date(GetYear(p_to_date), GetMonth(p_to_date)-1, GetDay(p_to_date));
...
function ShowWaitMsgWnd(pMesg) {
    try {
        ...
        m_div_mesg_line = document.createElement('div');
        m_div_mesg_line.style.height = "15px";
        m_div_mesg.appendChild(m_div_mesg_line);

        m_div_mesg_msg = document.createElement('div');
        m_Content = document.createTextNode(pMesg);
        m_div_mesg_msg.style.width = "300px";
        m_div_mesg_msg.appendChild(m_Content);
        m_div_mesg.appendChild(m_div_mesg_msg);

        m_div_mesg_line1 = document.createElement('div');
        m_div_mesg_line1.style.height = "15px";
        m_div_mesg.appendChild(m_div_mesg_line1);

    }
    catch (err) {
        alert("MsgBox() - Error In JScipt");
    }
}

function DestroyWaitMsgWnd() {
    try {
        p_WndCtrlRef = window.document.getElementById("xx");
        if (p_WndCtrlRef == null) {
            return;
        }
        ...
        alert("DestroyMsgBox() - Error In JScipt");
    }
}

function GetIdFromGridWithoutApp(p_grid_name, p_ctrl_name_id) {
    try {
        ...
        for (iRow = 0; iRow < m_grid_ctrl_ref.rows.length; iRow++) {
            ...
            if (m_ctrl_ref_id != null && m_ctrl_ref_id.value != "----") {
        ...
        alert('GetIdFromGridWithoutApp() - Error In JScipt');
        return "";
    }
}
...
//------------------------------------------
function Jf_OpenAsHtml(p_file_name, p_ParamList, p_sp_name, p_col_name, p_field_name, p_IsConnectToMasterDb) {
    try {
        ShowWaitMsgWnd("Please Wait");
        WebServiceMasters.ExportToExcel(p_file_name,
                                        p_ParamList,
                                        ...
                                        p_field_name,
                                        p_IsConnectToMasterDb,
                                        OnCompleteDynamicHtmlTemplate_OpenAsHtml,
                                        OnErrorDynamicHtmlTemplate_OpenAsHtml,
                                        OnTimeOutDynamicHtmlTemplate_OpenAsHtml
                                        );
    }
    catch (err) {
        alert("Jf_OpenAsHtml() - Error In JScipt");
    }
}
function OnCompleteDynamicHtmlTemplate_OpenAsHtml(arg) {
    DestroyWaitMsgWnd();
    if (arg > 0) {
        alert("Jf_OpenAsHtml() - Error In Web-Service Call");
    }
    else {
        ShowWndInMiddle("frmOpenAsHtml.aspx", 500, 500);
    }

}
function OnErrorDynamicHtmlTemplate_OpenAsHtml(arg) {
    alert("error has occured: " + arg._message);
}
function OnTimeOutDynamicHtmlTemplate_OpenAsHtml(arg) {
    alert("Time Out");
}

//------------------------------------------
function SetRadioButStat(m_curr_ctrl_name) {
    try {
        m_ref = window.document.getElementById(m_curr_ctrl_name);
        m_ref.checked = true;
    }
    catch (err) {
        alert('SetRadioButStat() - Error In JScipt');
        return false;
    }
}
...
function AddDate(p_date,p_day) {
    try {
        var m_Date1 = new Date(GetYear(p_date), GetMonth(p_date)-1, GetDay(p_date));
        m_Date1.setDate(m_Date1.getDate() + p_day);
...
function GetDayOfWeek(p_date) {
    try {
        var m_Date1 = new Date(GetYear(p_date), GetMonth(p_date) - 1, GetDay(p_date));
        m_str = m_Date1.toDateString();
        m_str = m_str.substr(0, 3);
        if (m_str == "Sun") {
            m_ret=1;
        }
        else if (m_str == "Mon") {
